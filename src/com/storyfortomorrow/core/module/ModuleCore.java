...
public class ModuleCore
{
	...
	{
		get().sftlogger.log(Level.INFO, "Loading modules...");
		for (Module module : found_modules)
			...
			{
				get().sftlogger.log(Level.SEVERE, "Duplicate instance of module id {0}", module.getInfo().id());
	...
	{
		get().sftlogger.log(Level.INFO, "Enabling modules...");
		for (Module module : loaded_modules)
		{
			...
			{
				get().sftlogger.log(Level.SEVERE, "Duplicate instance of Module: {0}", module.getInfo().id());
				this.disableModule(module);
			}
		}
	}
	...
	{
		get().sftlogger.log(Level.INFO, "Disabling modules...");
	...
	void loadModule(Module module) throws ModuleException
	{
		...
		finally
		{
			...
			get().sftlogger.log(Level.INFO, "Loaded module {0} v{1} id{2}!", new Object[]
	...
	void enableModule(Module module) throws ModuleException
	{
		...
		finally
		{
			...
			get().sftlogger.log(Level.INFO, "Enabled module {0} v{1} id{2}!", new Object[]
			{ module.getInfo().name(), module.getInfo().version(), module.getInfo().id()
			});
		}
	}
	...
	void disableModule(Module module) throws ModuleException
	{
		...
		{
			get().sftlogger.log(Level.INFO, "Disabled module {0} v{1} id{2}!", new Object[]
			{ module.getInfo().name(), module.getInfo().version(), module.getInfo().id()
			});
		}
	}
	...
	private List<Module> searchForModules(File file) throws ModuleException
	{
		...
		else get().sftlogger.log(Level.INFO, "Found {0} files... searching for modules...", classes.size());
		...
		get().sftlogger.log(Level.INFO, "Detected {0} modules!", modules.size());
